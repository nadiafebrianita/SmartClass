...
class Mhs extends CI_Controller {
	...
	public function update(){
		...
		$res1 = $this->m->update_data($where,$data,'mhs');
		...
		if(!empty($res1 && $res2)){
			...
			$this->session->set_flashdata('err', "Gagal Mengubah Data, NIM Sudah Terdaftar");
			redirect('mhs/index');
		}
	}
	...
	public function tambah_aksi(){
		...
		$res = $this->m_mhs->insert($data, $datascan);
		if(!empty($res)){
			$this->session->set_flashdata('true', "Berhasil Menambah Data"); 
			redirect('mhs/index');
		}
		else{
			$this->session->set_flashdata('err', "Gagal Menambah Data, NIM Sudah Terdaftar");
			redirect('mhs/index');
		}
	}
	public function hapus($nim)
	{
		...
		$res=$this->m->hapus_data($where,'mhs');
		if(!empty($res)){
	...
	public function form(){
		...
		//var_dump($data); die;
		$res = $this->m_mhs->insert_multiple($datascan,$data);
		if($res==true)
		{
